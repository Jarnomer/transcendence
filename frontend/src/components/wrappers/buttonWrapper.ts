type ButtonOptions = {
  ...
  export function createSVGButton(label: string, { id = '', type = 'button', className = ''}: ButtonOptions = {}) {
	...
	button.innerHTML = `${svg} <span class="relative text-primary ">${label}</span>`;
	button.classList.add('hover:scale-105', 'hover:shadow-lg', 'transition-all', 'duration-300', 'ease-in-out');
	return button;
  }
